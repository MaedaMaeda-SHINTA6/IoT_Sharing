  ...
  if (error) {
    ...
    request.post(requestData, function (error, response, body) {
      if (error) {
        ...
        // リクエスト作成
        var requestData = common.createGetRequest("matchings", null, "matchingName=", HEADERS);
          ...
          requestData, function (error, response, body) {
            if (error) {
              ...
              for (var index = 0; index < Itemjson.Matchings.length; index++) {
                ...
                // Item削除リクエスト作成
                var requestData = common.createDeleteRequest('matchings', Itemjson.Matchings[index].id, HEADERS);
                // リクエスト送信
                request.delete(requestData, function (error, response, body) {
                  if (error) {
                    common.outputError(error, null, null);
                    console.error('To delete the account(' + Itemjson.Matchings[index].id + ') has failed.');
                    console.error('Process has ended.');
