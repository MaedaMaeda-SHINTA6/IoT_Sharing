  ...
  if (error) {
    ...
    request.post(requestData, function (error, response, body) {
      if (error) {
        ...
        // リクエスト作成
        var requestData = common.createGetRequest("matching_statuses", null, "limit=10&offset=1", HEADERS);
          ...
          requestData, function (error, response, body) {
            if (error) {
              ...
              for (var index = 0; index < Itemjson.MatchingStatuses.length; index++) {
                ...
                // Item削除リクエスト作成
                var requestData = common.createDeleteRequest('matching_statuses', Itemjson.MatchingStatuses[index].id, HEADERS);
                console.log(requestData);
                // リクエスト送信
                request.delete(requestData, function (error, response, body) {
                  if (error) {
                    common.outputError(error, null, null);
                    console.error('To delete the account(' + Itemjson.MatchingStatuses[index].id + ') has failed.');
                    console.error('Process has ended.');
